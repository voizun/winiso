...
$TARGETS = @{
    ...
    "windows-11" = @{
        search = "windows 11, version amd64"
...
function Get-UupDumpIso($name, $target) {
    ...
    $result.response.builds.PSObject.Properties `
        ...
        | ForEach-Object {
            ...
            $editions = if ($langs -contains 'ja-jp') {
                ...
                Write-Host "Skipping. Expected langs=ja-jp. Got langs=$($langs -join ',')."
                [PSCustomObject]@{}
            }
            $_.Value | Add-Member -NotePropertyMembers @{
                editions = $editions
